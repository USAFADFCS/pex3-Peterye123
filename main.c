 ...
 * Documentation: I received help from Ethan Chapman on how to properly implement the tail->head search in pQAccess.
 * =========================================================== */
#include <stdio.h>
#include <stdlib.h>
#include "byutr.h"
...
int main(int argc, char **argv) {
    ...
    if (faults == NULL) {
        fprintf(stderr, "could not allocate faults array\n");
        pqFree(pq);
        fclose(ifp);
        exit(1);
    }
    ...
    while (!feof(ifp)) {
        ...
        if (depth == -1) {
            ...
            for (int frameCount = 1; frameCount < depth + 1; frameCount++) {
                faults[frameCount]++;
            }
        }
